...
<h4>Identificador {{receta.id_r}}</h4>
...
export default {
	...
	mounted() {
		axios.get('http://localhost:8083/bloquereceta/' + this.itemid_r)
		
		.then(response=> {
			this.receta = response.data
			
			axios.get('http://localhost:8083/ingredientesreceta/' + this.itemid_r)
			
			.then(response=> {
				this.ingredientes = response.data
				
				axios.get('http://localhost:8083/pasosreceta/' + this.itemid_r)
				
				.then(response=> {
					this.paso = response.data
				})
				
				.catch(error=> {
					alert("No se han podido cargar los pasos");
				})
			})
			
			.catch(error=> {
				alert("No se han podido cargar los ingredientes de la receta");
			})
	...
	methods: {
		...
		agregarReceta() {
			...
			axios.post('http://localhost:8083/modificarreceta', this.receta)
			.then(response=> {
				...
				for(var i=0; i < this.paso.length; i++) {
					this.paso[i].id_r = this.receta.id_r;
				}
						
				for(var j=0; j < this.ingredientes.length; j++) {
					this.ingredientes[j].id_re = this.receta.id_r;
				}
